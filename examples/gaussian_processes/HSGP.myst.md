...
```{code-cell} ipython3
pm.summary(idata, var_names=["eta", "ell", "sigma"])
```
...
$$
where $\Phi$ is the matrix of eigenvectors we use as the basis (plotted previously), and $\Delta$ has the spectral densities computed at the eigenvalues down the diagonal.  Below we show an example over with a two dimensional grid of input `X`.  It's important to notice that the HSGP approximation requires us to center the input `X` data, which is done by converting `X` to `Xs` in the code below.  We plot the approximate Gram matrix for varying $L$ and $c$ values, to see when the approximation starts to degrade for the given `X` locations and lengthscale choices.
...
f = pm.Deterministic("f", phi @ (beta * sqrt_psd)) 
```
where we use a $\text{Gamma}(\alpha=2, \beta=0.1)$ prior, which places around a 50% probability that $\nu > 30$, the point where a Student-T roughly becomes indistinguishable to a Gaussian.
...
```{code-cell} ipython3
pm.summary(idata, var_names=["beta0", "beta1", "eta", "ell", "sigma"])
```
...
ax1 = plt.subplot(131)
ax1.scatter(X_tr[:, 0], X_tr[:, 1], c=f_true[ix_tr])
...
for ax in axs:
    ax.set_xlabel("x")
    ax.set_ylabel("y")
axs[0].set_title("Underlying hierarchical GP for\nthe 10 time-series, mean GP in black")
axs[1].set_title("Observed data, color corresponding to GP");
```

To build this model to allow different lengthscales per GP, we need to rewrite the power spectral density.  The one attached to the PyMC covariance classes, i.e. `pm.gp.cov.Matern52.power_spectral_density`, is vectorized over the input dimension -- but we need one vectorized across GP.  Fortunately this one at least wasn't too hard to adapt.  It's shown below.
...
def hierarchical_HSGP(Xs, m, c, eta_mu, ell_mu, eta_delta, ell_delta):
    ...
    beta = pm.Normal("f_mu_coeffs_", mu=0.0, sigma=1.0, dims="m_ix")
    psd = matern52_psd(omega, ell_mu).flatten()
    f_mu = phi @ (beta * pt.sqrt(psd) * eta_mu)

    # calculate f_delta, the gp offsets
    beta = pm.Normal("f_delta_coeffs_", mu=0.0, sigma=1.0, dims=("m_ix", "gp_ix"))
...
with pm.Model(coords=coords) as model:
    ...
    f_mu, f_delta = hierarchical_HSGP(Xs, [m], c, eta_mu, ell_mu, eta_delta, ell_delta)
...
```{code-cell} ipython3
f_mu_mu = az.extract(idata, group="posterior", var_names="f_mu").mean(dim="sample")
f_mu_sd = az.extract(idata, group="posterior", var_names="f_mu").std(dim="sample")

f_inf_mu = az.extract(idata, group="posterior", var_names="f").mean(dim="sample")
f_inf_sd = az.extract(idata, group="posterior", var_names="f").std(dim="sample")
...
with model:
    ...
    idata.extend(
        pm.sample_posterior_predictive(
            thinned_idata, var_names=["f_delta_coeffs_", "f_mu_coeffs_", "f_mu", "f"]
        )
    )
```

```{code-cell} ipython3
f_mu_pp = az.extract(idata, group="posterior_predictive", var_names="f_mu").mean(dim="sample")
```
...
for ax in axs:
    ax.set_xlabel("x")
    ax.set_ylabel("y")
axs[0].set_title("Underlying Kronecker GP")
axs[1].set_title("Observed data, color corresponding to GP");
```
...
def kronecker_HSGP(Xs, m, c, cov_t, cov_x):
    ...
    z = pm.Normal("beta_", size=m_x * m_t)
...
```{code-cell} ipython3
%load_ext watermark
%watermark -n -u -v -iv -w -p pytensor,pymc,xarray
```
